...
class AppStore extends Component {
  state = {inputSearch: '', activeTab: tabsList[0].tabId}
  ...
  selectedTabItem = currentTab => {
    const selectedTab = tabsList.filter(each => each.tabId === currentTab)
    this.setState(prevState => ({
      inputSearch: prevState.inputSearch,
      activeTab: selectedTab[0].tabId,
    }))
  }

  render() {
    const {inputSearch, activeTab} = this.state
    const resultsTab = appsList.filter(each => each.category === activeTab)

    const finalResultsList = resultsTab.filter(each =>
